...
export function FriendDetail({ users, friendId, balances, expenses, payments, currentUserId, onBack, onSettleUp, onEditExpense, onRemoveExpense }: FriendDetailProps) {
  ...
  const sharedPayments = payments.filter(p =>
        ...
        <div className="mt-4">
          {Math.abs(amount) < 0.01 ? (
            ...
            <p className="text-lg">
              ...
              <span className="font-bold text-danger">{formatCurrency(Math.abs(amount))}</span>
            </p>
          )}
          <p className="text-xs text-charcoal-light mt-1">(balance shown in USD)</p>
        </div>
        {amount < -0.01 && (
          <Button className="mt-4" onClick={handleSettleUp}>
      ...
      <Card className="p-4">
        ...
        <div className="space-y-3">
          {sharedExpenses.map(expense => {
            const paidByName = expense.paidBy === currentUserId ? 'You' : users.find(u => u.id === expense.paidBy)?.name
            return (
              <div key={expense.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-charcoal-light">{paidByName} paid {formatCurrency(expense.amount, expense.currency)} · {formatDate(expense.date)}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-sm text-charcoal">
                    {payment.fromUserId === currentUserId ? `You paid ${friend.name}` : `${friend.name} paid you`}
                  </p>
                  <p className="text-xs text-charcoal-light">{formatCurrency(payment.amount, payment.currency)} · {formatDate(payment.date)}</p>
                </div>
              </div>
            </div>
          ))}
          {sharedExpenses.length === 0 && sharedPayments.length === 0 && (
            <p className="text-center text-charcoal-light py-4 text-sm">No shared expenses yet</p>
          )}
        </div>
      </Card>
    </div>
