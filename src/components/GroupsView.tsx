...
interface GroupsViewProps {
  ...
  getGroupSimplifiedDebts: (
    group: Group,
  ) => { from: string; to: string; amount: number; explanation: string[] }[];
...
export function GroupsView({
  ...
  const [formRoles, setFormRoles] = useState<Record<string, GroupRole>>({
    [currentUserId]: "admin",
  });
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-charcoal-light mt-1">
          Balances in this group will be shown in this currency
        </p>
      </div>
      ...
      <div>
              ...
              {formMembers.includes(friend.id) && (
                <select
                  ...
                  onChange={(e) =>
                    setFormRoles((prev) => ({
                      ...prev,
                      [friend.id]: e.target.value as GroupRole,
                    }))
                  }
        ...
        <p className="text-xs text-charcoal-light mt-1">
          Admin: full access. Expenses only: can only add expenses.
        </p>
      </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-charcoal-light">
                    {memberCount} members · {currencySymbols[group.currency]}{" "}
                    {group.currency}
                  </p>
                </div>
              </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-charcoal">
            ...
            <strong>{groups.find((g) => g.id === confirmDelete)?.name}</strong>?
